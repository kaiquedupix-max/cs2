...
namespace Mac1ota_Menu.Modules.Legit
{
    public class FovChanger : Classes.ThreadService, IModule // hella detected
    {
        ...
        {
            if (!Enabled || GameState.LocalPlayer == null || GameState.LocalPlayer.PawnAddress == IntPtr.Zero || GameState.memory == null || GameState.LocalPlayer.Health == 0)
                return;
            ...
            GameState.IsScoped = GameState.memory.ReadBool(GameState.LocalPlayer.PawnAddress, Offsets.m_bIsScoped); // get scoped status
            GameState.CameraServices = GameState.memory.ReadPointer(GameState.LocalPlayer.PawnAddress, Offsets.m_pCameraServices);
